...
- [โมดูลงานวิจัย](#โมดูลงานวิจัย)
- [การใช้งานสำหรับผู้สูงอายุ](#การใช้งานสำหรับผู้สูงอายุ)
- [การพัฒนาและทดสอบ](#การพัฒนาและทดสอบ)
- [ตัวชี้วัดของการศึกษา](#ตัวชี้วัดของการศึกษา)
...
| **งานวิจัย** | จัดการผู้เข้าร่วม กลุ่มทดลอง/ควบคุม การให้ความยินยอม การถอนตัว · คำนวณผลลัพธ์ก่อน/หลังอัตโนมัติ · ตารางสถิติพร้อม p-value · ส่งออกข้อมูลแบบไม่ระบุตัวตนพร้อม codebook |
...
| **ความเร็วในการทำงาน** | ค้นหาด่วนด้วย `⌘K` / `/` · แจ้งผลแบบ toast · กันการกดบันทึกซ้ำ · ปุ่มลัด (`?` ดูทั้งหมด) |
...
| **การเข้าถึงสำหรับผู้สูงอายุ** | ปรับขนาดตัวอักษร 3 ระดับ · โหมดตัดกันสูง · ปุ่มกดขนาดใหญ่ · ระบบจำค่าที่ตั้งไว้ในเครื่องผู้ป่วย |
...
│   ├── research.py            # โดเมน: งานวิจัย (กลุ่มศึกษา ช่วงเก็บข้อมูล ผลลัพธ์)
│   ├── stats.py               # สถิติ: t-test, Wilcoxon, McNemar, Chi-square (ไม่ใช้ SciPy)
│   ├── scheduler.py           # งานตามเวลา + กันงานซ้ำเมื่อมีหลาย worker
│   └── routers/               # auth, dashboard, patients, reports, research,
│                              # symptoms, admin, api, qr, public, line_bp
├── static/                    # CSS, ฟอนต์ไทย, Chart.js, สคริปต์ UX (self-hosted ทั้งหมด)
...
| Frontend | Jinja2 + CSS/JS ที่ host เอง (ไม่พึ่ง CDN), Chart.js, ฟอนต์ Sarabun |
...
| Statistics | Python standard library (ไม่พึ่ง SciPy/NumPy) |
...
ระบบถูกออกแบบให้เป็นเครื่องมือเก็บข้อมูลของงานวิจัยโดยตรง
ผลลัพธ์ทางคลินิก (ความสม่ำเสมอ, TTR, สัดส่วน INR ในเป้าหมาย)
**คำนวณจากข้อมูลที่เภสัชกรบันทึกอยู่แล้ว** ไม่ต้องกรอกซ้ำ

### ขั้นตอนการใช้งาน

1. **รับผู้เข้าร่วม** — เปิดหน้าข้อมูลผู้ป่วย กด "เพิ่มเข้างานวิจัย"
   บันทึกกลุ่มศึกษา (ทดลอง/ควบคุม) วันที่ลงนามยินยอม และเวอร์ชันเอกสารยินยอม
2. **กำหนดช่วงเก็บข้อมูล** — ระบุวันเริ่ม Baseline และความยาวช่วง (ค่าเริ่มต้น 60 วัน)
   ระบบคำนวณช่วง Endline ต่อเนื่องให้อัตโนมัติ หรือกำหนดเองก็ได้
3. **บันทึกแบบวัด** — คะแนนความรู้, ความร่วมมือใช้ยา, ความพึงพอใจ, คุณภาพชีวิต
   บันทึกได้ทั้งช่วง Baseline และ Endline (บันทึกซ้ำจะแทนที่ค่าเดิม)
4. **ติดตามความครบถ้วน** — หน้า `/research` แสดงรายชื่อผู้ที่ยังเก็บข้อมูลไม่ครบ
5. **วิเคราะห์และส่งออก** — หน้า `/research/analysis` สร้างตารางพร้อมใช้ในรายงาน

### ตารางที่ระบบสร้างให้

| ตาราง | เนื้อหา | สถิติที่ใช้ |
|-------|---------|------------|
| ตารางที่ 1 | ลักษณะพื้นฐานของผู้เข้าร่วม แยกตามกลุ่ม | Welch's t-test, Chi-square |
| ตารางที่ 2 | เปรียบเทียบก่อน/หลังใช้ระบบ | Paired t-test + Wilcoxon signed-rank |
| ตารางที่ 3 | สัดส่วนผู้ผ่านเกณฑ์ (สม่ำเสมอ ≥80%, TTR ≥65%) | McNemar's test, Wilson 95% CI |
| ตารางที่ 4 | เปรียบเทียบระหว่างกลุ่มที่ Endline | Welch's t-test |

ทุกตารางรายงาน mean ± SD, 95% CI, ค่า p และ effect size (Cohen's d / Cramér's V)

### การส่งออกข้อมูล

| ไฟล์ | เนื้อหา |
|------|---------|
| `export.csv` | ข้อมูลรายบุคคล 1 แถวต่อผู้เข้าร่วม พร้อมคอลัมน์ผลต่าง (endline − baseline) |
| `export.xlsx` | สมุดงาน 3 แผ่น: `data`, `codebook`, `results` |
| `codebook.csv` | พจนานุกรมตัวแปรสำหรับ SPSS / R / Stata |

> **ไฟล์เป็นแบบไม่ระบุตัวตนเสมอ** — ไม่มี HN และชื่อผู้ป่วย
> เฉพาะผู้ดูแลระบบเท่านั้นที่ดาวน์โหลดแบบระบุตัวตนได้ (`?identified=1`)
> เมื่อจำเป็นต้องตรวจสอบข้อมูลย้อนกลับ และการดาวน์โหลดทุกครั้งถูกบันทึกใน audit log

### หมายเหตุทางสถิติ

สถิติทั้งหมดเขียนด้วย Python มาตรฐาน ไม่พึ่ง SciPy/NumPy
(ตรวจสอบความถูกต้องด้วยการเทียบกับการหาปริพันธ์เชิงตัวเลขและสูตรปิดใน `tests/test_stats.py`)

ผลที่ได้เหมาะสำหรับติดตามความคืบหน้าและร่างรายงาน
**ก่อนตีพิมพ์ควรตรวจสอบสมมติฐานของสถิติ** (การแจกแจง ค่าผิดปกติ ข้อมูลขาดหาย)
ด้วยโปรแกรมสถิติเฉพาะทาง โดยใช้ไฟล์ที่ส่งออกจากระบบ

---

## การใช้งานสำหรับผู้สูงอายุ

ผู้ป่วยที่ใช้วาร์ฟารินส่วนใหญ่อายุมากกว่า 60 ปี หน้าเว็บฝั่งผู้ป่วยจึงออกแบบดังนี้

- **ปรับขนาดตัวอักษรได้ 3 ระดับ** (ปกติ / ใหญ่ / ใหญ่มาก) ด้วยปุ่มเดียวที่ด้านบนทุกหน้า
- **โหมดตัดกันสูง** เพิ่มความเข้มของตัวอักษรและเส้นขอบ สำหรับผู้ที่สายตาไม่ดีหรืออ่านกลางแดด
- **ระบบจำค่าที่ตั้งไว้** ในเครื่องของผู้ป่วย ตั้งครั้งเดียวใช้ได้ตลอด
- **ปุ่มกดขนาดใหญ่** (สูงอย่างน้อย 46 px) และช่องติ๊กขนาด 22 px
- **ป้องกันการกดยืนยันซ้ำ** ปุ่มจะปิดตัวเองทันทีหลังกดครั้งแรก
- **ไม่ต้องล็อกอิน** ทุกหน้าเปิดจากลิงก์เฉพาะบุคคลใน LINE ได้ทันที

ฝั่งเจ้าหน้าที่รองรับ skip link, ARIA landmarks, focus ring ที่มองเห็นชัด
และ `prefers-reduced-motion` สำหรับผู้ที่ไวต่อการเคลื่อนไหว

## บทบาทผู้ใช้งาน

| บทบาท | สิทธิ์ |
|-------|-------|
| `admin` ผู้ดูแลระบบ | ทุกอย่าง + จัดการบัญชีผู้ใช้ + บันทึกการใช้งาน + สถานะระบบ + ส่งออกข้อมูลวิจัยแบบระบุตัวตน |
| `pharmacist` เภสัชกร | แก้ไขข้อมูลผู้ป่วย แผนยา ผล INR นัดหมาย ตอบอาการ + จัดการผู้เข้าร่วมวิจัย |
| `nurse` พยาบาล | เหมือนเภสัชกร ยกเว้นการรับผู้เข้าร่วมวิจัย |
...
| `test_stats.py` | สถิติทุกฟังก์ชัน เทียบกับการหาปริพันธ์เชิงตัวเลขและสูตรปิด |
| `test_research.py` | การรับผู้เข้าร่วม, ผลลัพธ์รายช่วง, ตารางวิเคราะห์, การส่งออกและการปกปิดตัวตน |
| `test_ux.py` | สคริปต์และสไตล์ UX, skip link, ARIA, ขนาดปุ่มกด, โหมดตัดกันสูง |

---

## ตัวชี้วัดของการศึกษา
...
ระบบมีโมดูลงานวิจัยในตัว — ดูรายละเอียดที่หัวข้อ [โมดูลงานวิจัย](#โมดูลงานวิจัย)

**ตัวชี้วัด (ดาวน์โหลดได้จากหน้ารายงานและหน้าวิเคราะห์ผล)**
